...
async function sendImageHandle(url,user_id) {
  client.sendMessageText(user_id,"图片下载转发中，...")
...
//ImageVariation
async function ImageVariation(file){
  const { Configuration, OpenAIApi } = require("openai");
  const configuration = new Configuration({
   apiKey: `${config.API_KEY}`
   });    
  ...
  const buffer = file;
  // Set a `name` that ends with .png so that the API knows it's a  PNG 
 image
 ...
 //from url get url
 async function getImageFromUrl2url(url,user_id) {
     return new Promise((resolve, reject) => {
       https.get(url, (res)=> {
           let data = new Buffer.from([]);
           ...
           res.on('end', async () => {
             let Newurl = await ImageVariation(data)
             console.log(Newurl)
             resolve(Newurl)
               //const {url}  = await client.uploadFile(data)
               //resolve(url);
           });
       });
     });
   }
 // GET Attachment and send Vanrition IMAGE
 async function GetAttachment2Vanrition(msg){
   let url = await client.showAttachment(msg.data.attachment_id).
   then(async (messageInfo)=>{ 
     console.log(messageInfo)
     await client.sendMessageText(msg.user_id,messageInfo.view_url)
     client.sendMessageText(msg.user_id,"图片正在处理中，请稍等...")
     let AI_url = await getImageFromUrl2url(messageInfo.view_url,msg.
   user_id)
    await client.sendMessageText(msg.user_id,AI_url)
     await sendImageHandle(AI_url,msg.user_id)
   })
 }